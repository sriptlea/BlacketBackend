...
export class AuctionsService {
    ...
    async bid(userId: string, id: number, dto: AuctionsBidAuctionDto, ip: string) {
        ...
        if (dto.amount <= highestPreviousBidAmount) throw new ForbiddenException(
        ...
        if (user.diamonds < additionalAmount) throw new ForbiddenException(Forbidden.AUCTIONS_BID_NOT_ENOUGH_DIAMONDS);

        if (auction.price >= dto.amount) throw new ForbiddenException(
            Forbidden.AUCTIONS_BID_TOO_LOW.replace("%s", (auction.price + 1).toLocaleString())
        );

        if (auction.bids.length > 0 && auction.bids[0].amount >= dto.amount) throw new ForbiddenException(
