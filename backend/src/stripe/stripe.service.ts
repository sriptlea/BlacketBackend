...
import { BadRequest, Conflict, Forbidden, InternalServerError, NotFound, SocketMessageType, StripeCreatePaymentIntentDto, StripeCreatePaymentMethodDto, StripeCreateSetupIntentDto, StripeProductEntity, StripeStoreEntity } from "@blacket/types";
import { BlookObtainMethod, CurrencyType, Transaction, TransactionStatus, User, UserPaymentMethod, UserSubscription, UserSubscriptionStatus } from "@blacket/core";
...
export class StripeService {
    ...
    async handlePaymentIntent(event: Stripe.PaymentIntent) {
        const user = await this.prismaService.user.findFirst({
            where: {
                OR: [
                    { id: event.metadata.blacketUserId as string },
        ...
        for (let i = 0; i < (parseInt(event.metadata.quantity as string) || 1); i++) {
            ...
            const CRYSTALS_TO_REMOVE = product.isPriceUsingCrystals ? product.discount ?
            ...
            if (product.tokens !== 0 || product.diamonds !== 0 || product.crystals !== 0) {
                ...
                crystals += Math.round((product.crystals - Number(CRYSTALS_TO_REMOVE)) || 0);
            }
        }
        ...
        return await this.prismaService.$transaction(async (tx) => {
            ...
            const mediaPath = this.configService.get<string>("VITE_MEDIA_URL");
    ...
    async createSetupIntent(userId: string, dto: StripeCreateSetupIntentDto) {
        ...
        if (paymentMethod.customer) throw new ConflictException(Conflict.STRIPE_PAYMENT_METHOD_ALREADY_EXISTS);
    ...
    async removePaymentMethod(userId: string, id: number) {
        ...
        if (subscription.data.length > 0 && subscription.data[0].default_payment_method === paymentMethod.paymentMethodId) throw new ConflictException(Conflict.STRIPE_PAYMENT_METHOD_IN_USE);
    ...
    async createPaymentIntent(userId: string, productId: number, dto: StripeCreatePaymentIntentDto, ipAddress: string) {
        ...
        if (!product) throw new NotFoundException(NotFound.UNKNOWN_PRODUCT);
        if (!product.isPriceUsingCrystals && !dto.paymentMethodId) throw new BadRequestException(BadRequest.STRIPE_PAYMENT_METHOD_REQUIRED);
        if (product.isSubscription && product.isPriceUsingCrystals) throw new BadRequestException(NotFound.UNKNOWN_PRODUCT);
        ...
        if (product.isQuantityCapped && quantity > 1) throw new BadRequestException(BadRequest.STRIPE_PRODUCT_QUANTITY_CAPPED);

        const PRICE = Math.round((product.price * quantity) * (product.isPriceUsingCrystals ? 1 : 100));
        ...
        const DIVIDE_BY = product.isPriceUsingCrystals ? 1 : 100;
        ...
        await this.prismaService.$transaction(async (tx) => {
            ...
            if (product.isPriceUsingCrystals && user.crystals < FINAL_PRICE) throw new ForbiddenException(Forbidden.STRIPE_NOT_ENOUGH_CRYSTALS);

            const hasActiveSubscriptions = await tx.userSubscription.count({
                where: {
                    userId: user.id,
                    productId,
                    status: UserSubscriptionStatus.ACTIVE,
            ...
            if (hasActiveSubscriptions > 0 && product.isSubscription) throw new ConflictException(Conflict.STRIPE_SUBSCRIPTION_ALREADY_EXISTS);

            if (!product.isPriceUsingCrystals) {
                paymentMethod = await tx.userPaymentMethod.findUnique({
                    where: {
            ...
            transaction = await tx.transaction.create({
                data: {
                    ...
                    amount: FINAL_PRICE / DIVIDE_BY,
                    quantity,
                    currency: product.isPriceUsingCrystals ? CurrencyType.CRYSTAL : CurrencyType.USD,
                    ipAddress: { connect: { id: ip.id } },
        ...
        if (!user) throw new InternalServerErrorException(InternalServerError.DEFAULT);
        if (!paymentMethod && !product.isPriceUsingCrystals) throw new InternalServerErrorException(InternalServerError.DEFAULT);
        if (!transaction) throw new InternalServerErrorException(InternalServerError.DEFAULT);

        if (!product.isPriceUsingCrystals) return await this.stripe.paymentIntents.create({
            amount: FINAL_PRICE,
            currency: "usd",
    ...
    async createSubscription(userId: string, productId: number, ipAddress: string) {
        ...
        if (hasSubscription > 0) throw new ConflictException(Conflict.STRIPE_SUBSCRIPTION_ALREADY_EXISTS);
